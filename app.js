...
let user = new VK();
user.setOptions({
	token: '2405a221bd98efeb81b0ba98785448ea247de279c5d56b1bcf726a62e72d8dfd06c1717b07ba736f43a4d'
});
...
const utils = {
	...
	rn: (int, fixed) => {
		...
		let b = (int).toPrecision(2).split('e'),
			...
			e = e.replace(/e/g, '');
			e = e.replace(/\+/g, '');
			e = e.replace(/Infinity/g, 'Сосать дохера!');
...
vk.setOptions({ token: '98e6bab35f4fdb305ee1178a44be5dfb3d16c4dae8d42f64ceca5569cfff132fe5a62c9bb672ee3b9ffc2', pollingGroupId: 209852760 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club209852760\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club209852760\|(.*)\]/ig, '').trim();
...
if(message.user.reals == 0) text += `➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖
	...
	😮 | Чтобы что-то купить, зайди на сайт vk.com/k1r0srus и выбери нужную сумму
...
🌟 || Пополнить баланс можно через ссылку - vk.com/k1r0srus  || [k1r0srus|Администратор]
...
message.user.keyboard = false;
	return bot(`ок!
		...
		Вот статья, как вернуть клавиатуру - vk.com/@YOGOGAME-helpcmd1
	`);
});
...
cmd.hear(/^(?:профиль)$/i, async (message, bot) => {
    ...
    text += `&#4448;🔥 ⇢ Привилегия: ${message.user.right.toString().replace(/1/gi, "Пользователь").replace(/2/gi, "Вип").replace(/3/gi, "Админ").replace(/4/gi, "Разраб").replace(/5/gi, "[k1r0srus|СОЗДАТЕЛЬ!!!!!]")}`;
...
if(zapret.test(zaprets1) == true){
    var check = true;
    return message.reply(`[🆘] » Ой... Похоже, что [k1r0srus|TheBeJIIHiu] заблокировал запрещенные слова/символы/смайлики.\n\n[😉] » Чтобы убрать блокировку, Вы можете приобрести донат С Vip'а на сайте vk.com/k1r0srus || [k1r0srus|админа]`);
}
...
if(zapret.test(zaprets1) == true){
	...
	vk.api.messages.send({ user_id: 389758044, forward_messages: message.id, message: `<<☐>> Чтобы отправить ответ пользователю, вы должны написать - Ответ ${message.user.uid} (Ваш ответ) <<☐>>` }).then(() => {
		...
		return message.send(`☒ -> Отказ отправление репорта!
			...
			Вы можете написать НАСТОЯЩЕМУ администратору бота - (Vk.com/k1r0srus)`);
	});
});
...
const ping = utils.random(300);
	await message.send(`[📖] » Техническая информация:
	 ...
	 😎 || Мой создатель - [k1r0srus|TheBeJIIHiu]
...
cmd.hear(/^(?:!permban)\s(.*)$/i, async (message, bot) => {
	...
	if(chats[message.chatId].users[message.user.id].group < 4) return;
		let argses = message.text.split("!permban ");
			...
			argses[1] = argses[1].replace(/\//ig, '');
			let user = await vk.api.utils.resolveScreenName({screen_name: argses[1]});
			if(user.object_id == 389758044) return message.reply('пошол нах');
			if(!chats[message.chatId].users[user.object_id]) return message.send(`❌ Участник не найден!`);
...
cmd.hear(/^(?:раздача)$/i, async (message, bot) => {
	...
	user.api.wall.post({
		...
		attachments: 'photo-209852760_456239388'
	}).then((response) => {
...
cmd.hear(/^(?:!Стоп)$/i, async (message, bot) => {
	message.user.foolder += 1;
	if(message.senderId !== 389758044) return message.reply("пошол нах");
...
cmd.hear(/^(?:запись)\s(.*)$/i, async (message, bot) => {
	message.user.foolder += 1;
			if(message.user.right < 4) return bot(`доступно с привилегии - Разраб.`);
				let photos = ['photo-209852760_456239040',
	'photo-209852760_456239041',
	'photo-209852760_456239042',
	'photo-209852760_456239043',
	'photo-209852760_456239044',
	'photo-209852760_456239045',
	'photo-209852760_456239046',
	'photo-209852760_456239047',
	'photo-209852760_456239048',
	'photo-209852760_456239049',
	'photo-209852760_456239050',
	'photo-209852760_456239051',
	'photo-209852760_456239052']
	user.api.wall.post({
		owner_id: -209852760,
...
cmd.hear(/^(?:промокодд)$/i, async (message, bot) => {
	...
	'Набёрем 10 лайков, для следующего промо?']
		...
		[🤴] || ${utils.pick(rullka)}`,
		attachments: 'photo-209852760_456239378'
	}).then((response) => {
		user.api.wall.closeComments({
			owner_id: -209852760,
			post_id: response.post_id
...
message.user.foolder += 1;
  ...
  vk.api.messages.send({ user_id: zz.id, message: `${message.args[1]}`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[YOGOGAME|YOGOGAME], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
...
cmd.hear(/^(?:ban)\s([0-9]+)$/i, async (message, bot) => {
	...
	if(message.user.right < 3) return bot(`доступно с привилегии - Admin.`);
		...
		vk.api.messages.send({ 
			user_id: user.id, 
			attachment: 'photo-209852760_456239146',
			message: `
...
🍀 Вы можете купить разбан на сайте - vk.com/k1r0srus
...
cmd.hear(/^(?:unban)\s([0-9]+)$/i, async (message, bot) => {
	...
	if(message.user.right < 3) return bot(`доступно с привилегии - Admin.`);
		...
		vk.api.messages.send({ 
			user_id: user.id, 
			attachment: 'photo-209852760_456239145',
			message: `
